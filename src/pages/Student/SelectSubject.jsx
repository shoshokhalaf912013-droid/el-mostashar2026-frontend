...
export default function SelectSubject() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (currentUser) => {
      ...
      const gradeId = storedUser?.gradeId;
  ...
  return (
    <div className="min-h-screen bg-black text-white px-6 py-10">
      ...
      {Object.keys(tracks).length > 0 && (
        <div className="mb-16">
          <h2 className="text-xl text-center mb-6 text-yellow-300">🧭 المسارات</h2>
          {Object.values(tracks).map((track) => (
            <Section
              key={track.label}
              title={track.label}
              items={track.subjects}
            />
          ))}
        </div>
      )}
      ...
      {commonNotAdded.length > 0 && (
        <Section title="📕 مواد لا تُضاف للمجموع" items={commonNotAdded} dimmed />
      )}
    </div>
  );
}
...
function Section({ title, items, dimmed }) {
  return (
    <div className="mb-10">
      <h3 className="text-lg text-center mb-4 text-yellow-300">{title}</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {items.map((item) => (
          <div
            key={item}
            className={`p-6 rounded-xl text-center border ${
              dimmed ? "border-gray-700 opacity-70" : "border-yellow-600"
            }`}
          >
            {item}
          </div>
